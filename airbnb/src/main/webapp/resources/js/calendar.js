...
function gotoReservation(){
	...
	}else if(r_date_list.length < 2 ){
		alert("숙박 일정은 최소 2일 이상 가능합니다.");
	}else{
		isTrue = true;
	}
	...
	if(isTrue){
		...
		$("html, body").animate({scrollTop: $("#datePicker_div").offset().top - 100},400);
	}
}
...
function getDateArray(startDate, endDate){
	...
	while(curDate <= end){
		var fullDay = curDate.getFullYear() + "-" + (curDate.getMonth()+1).toString().padStart(2, "0") + "-" + curDate.getDate().toString().padStart(2, "0");
		arr.push(fullDay);
